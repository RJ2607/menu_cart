...
/// [itemTemplate] via `` placeholders, plus the special ``
...
class StReorderableListViewBuilder extends StacWidget {
  ...
  /// Stac widget rendered for every item. `` placeholders are
  /// replaced with the item's fields, and `` with its position.
  ///
  /// Can be a typed Stac widget (e.g. `StacText(data: '')`) or a raw
  /// JSON map from a server payload (e.g. `{"type": "text", "data": ""}`).
  ...
  /// `` and `` placeholders are replaced with the
  /// source and destination positions, and `` with the moved item's
  /// position.
  ///
  /// ```json
  /// "onReorder": {
